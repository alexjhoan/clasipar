...
const index = ({data}) => {
	const [rows, setRows] = useState([['ad_id','corporate_name','user_id','user_email','ad_supplier','ad_location','ad_cellphone_number','ad_cellhopne_number2','ad_phone_number']])
	...
	const DownloadData = () => {
		data.list.map((item) => {
			rows.push([
				...
				item.user_email,
				item.ad_supplier,
				item.ad_location.map(i => {return(i.location_name)}).join(" - "),
				item.ad_metadata.ad_cellphone_number ? item.ad_metadata.ad_cellphone_number.value : '',
				item.ad_metadata.ad_cellhopne_number2 ? item.ad_metadata.ad_cellhopne_number2.value : '',
	...
	const ConvertData = () => {
		...
		link.setAttribute("download", "Clasipar.csv");
